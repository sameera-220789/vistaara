...
def process_geotiff(
    ...
    is_sen2sr = model_name.lower() in ["sen2sr", "sen2sr_lite", "opensr", "opengeoai"]
    if is_sen2sr:
        log("Sen2SR ESA OpenSR Sentinel-2 super-resolution requested.")
        log("Activating 2.5m GSD target with physical radiometric hard-constraint layer.")
        sr_bgr, model_used, is_ai, sr_logs = run_realesrgan_inference(bgr_uint8, scale=scale, model_name="sen2sr", log_callback=log)
        scientific_notice = "Sen2SR Hard-Constraint: Sentinel-2 mean surface reflectance strictly conserved."
        multispectral_label = "Sen2SR (ESA OpenSR 2.5m Sentinel-2 Super-Resolution)"
    else:
